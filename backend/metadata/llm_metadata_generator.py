...
try:
    from metadata.gemini_client import GeminiClient
...
class LLMMetadataGenerator(BaseMetadataGenerator):
    """Metadata generator using LLM (supports Azure OpenAI and Gemini)."""
    ...
    def __init__(self, output_dir="metadata_gen_output"):
        ...
        try:
            ...
            self.logger.info(f"Initializing LLM with provider: {provider}")

            if provider == "gemini":
                if not GEMINI_AVAILABLE:
                    raise ImportError("Gemini client not available. Install google-generativeai")
