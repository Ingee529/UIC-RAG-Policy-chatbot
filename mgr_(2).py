...
def scrape_section_with_subsections(root_url, section_title, session):
    ...
    if len(root_content) < 100:
        print("  ⚠ Short content in root page, will look for subsections")
...
print(f"\n✓ Saved {len(df)} pages to {csv_path}")
...
# === PART 1: PARSING ===
# ✅ Unified code for local files + scraped pages
...
for filename in uploaded_files:
    ...
    try:
        ...
        print(f"✅ Parsed file: {filename} → {len(blocks)} blocks")
    except Exception as e:
...
print(f"🌐 Parsed {len(scraped_blocks)} blocks from {len(scraped_pages)} scraped pages")
...
for idx, (url, blocks) in enumerate(grouped_by_page.items()):
    ...
    print(f"✅ Added scraped page: {doc_key} → {len(blocks)} blocks")
...
try:
    ...
    raise ValueError("⚠️ `final_chunks` not found. Please run Part 2 (chunking) before this.")
